...
Uses the same logic as easy_tender.format_requirements (library used by the web app).

Example (from repo root):
  python step1/export_formated_requirements.py path/to/extraction.json
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Write normalized requirements JSON into step1/ (same folder as this script)."
    )
    parser.add_argument(
        "input",
        type=Path,
